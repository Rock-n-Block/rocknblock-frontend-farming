...
const ContactUs: FunctionalComponent<ContactUsProps> = ({
    ...
    const fetchForm = (token: string): void => {
        if (token) {
            fetch(
                'https://rocknblock.io/api/v1/send_unblocking_feedback/',
                {
                    method: 'POST',
                    headers,
                    body: formData
    ...
    return (
        ...
        >
            ...
            <form
                ...
                className={style['contact-us__form']}
            >
                <input
                    type='hidden'
                ...
                <button
                    type='submit'
                    className={`${style['contact-us__form__submit-btn']} ${style.btn} ${style['btn-orange']} ${style['btn-rounded']}`}
                >
                    Submit request
                </button>
            ...
            <div
                id='contact-us-popup'
                className={`${style['contact-us__popup-container']} ${
                    isSubmitted ? style.show : null
                }`}
            ...
            >
                <div className={`${style['contact-us__popup']} ${style.popup}`}>
                    ...
                    <div
                        className={`${style['background-img']} ${style['background-img-left']}`}
                    >
                        <img
                            src='../../assets/img/background-popup-left.png'
                            alt='background-popup-left'
                        />
                    </div>
                    <div
                        className={`${style['background-img']} ${style['background-img-right']}`}
                    >
                        <img
                            src='../../assets/img/background-popup-right.png'
                            alt='background-popup-right'
                        />
                    </div>
                </div>
            </div>
